          ...
          <main className="flex-1 p-6 lg:p-8 max-w-[1400px] animate-in fade-in slide-in-from-bottom-1 duration-400">
            ...
            {viewMode === "card" && (
              <div className="flex flex-col gap-3">
                {blocs?.map((bloc) => (
                  <div key={bloc.id} className="bg-card border border-border rounded-xl shadow-[var(--shadow-card)] overflow-hidden hover:shadow-[var(--shadow-elevated)] transition-shadow duration-300 flex items-center gap-4 p-4 cursor-pointer" onClick={() => handleBlocClick(bloc)}>
                    <div className="w-10 h-10 rounded-lg bg-accent/10 flex items-center justify-center text-accent shrink-0">
                      <Box className="w-5 h-5" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="font-display text-sm font-bold leading-tight truncate">{bloc.name}</h3>
                      <p className="text-xs text-muted-foreground mt-0.5 truncate">{bloc.projectName} · {bloc.trancheName}</p>
                    </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Project / Tranche</TableHead>
                  ...
                  <TableBody>
                    {blocs?.map((bloc) => (
                      <TableRow key={bloc.id} className="cursor-pointer" onClick={() => handleBlocClick(bloc)}>
                        <TableCell className="font-semibold">{bloc.name}</TableCell>
                        <TableCell className="text-muted-foreground">
                          {bloc.projectName} / {bloc.trancheName}
                        </TableCell>
                        <TableCell>
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                            bloc.status === 'active' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400' : 'bg-muted text-muted-foreground'
                        ...
                        <TableCell>{bloc.floors}</TableCell>
